...
enum WgIdent { case none,refresh,reset,saveLoad,recSaveLoad,help,stereo,burningShip,resolution,julia,move,rotate,texture }
...
class WidgetGroup: NSView {
    ...
    func hotKey(_ s:String) {
        ...
        for i in 0 ..< data.count {
            if s == data[i].hotKey {
                focus = i
                if data[focus].kind == .command { delegate?.wgCommand(data[focus].ident) }
                if data[focus].kind == .toggle  { delegate?.wgToggle(data[focus].ident) }
                refresh()
                return
            }
        }
    ...
    func drawEntry(_ index:Int) {
        ...
        case .singleFloat, .dualFloat, .float3Dual, .float3Single, .float3xy, .float3z : drawGraph(index)
    ...
    func update() -> Bool {
        ...
        default :
            if data[focus].isValueWidget() {
                ...
                if data[focus].ident == .move {
                    vc.alterPosition(delta.x,delta.y,0)
                    return true
                }
                else  if data[focus].ident == .rotate {
                    vc.alterAngle(delta.x,delta.y)
                    return true
                }
                else { return false }
        }
    ...
    func moveFocus(_ dir:Int) {
        ...
        func move() {
            while true {
                ...
                if [ .move, .rotate ].contains(data[focus].ident) { break }
            }
        }
    ...
    func focusMovement(_ pt:CGPoint, _ touchCount:Int) {
        ...
        delta.x =  Float(pt.x) / 100
        delta.y = -Float(pt.y) / 100
